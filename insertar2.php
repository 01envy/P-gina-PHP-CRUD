...
if (count($errors) > 0) {
    ...
    header("Location: registro.php?error=" . urlencode(implode(", ", $errors)));
    exit;
}
...
if ($query) {
    header("Location: registro.php?success=Te has registrado con éxito!");
} else {
    header("Location: registro.php?error=base_de_datos");
}
?>